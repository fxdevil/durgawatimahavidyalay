...
export function mountAddStudents(main) {
  ...
  function showPlan() {
    ...
    preview.querySelector('#import-students').addEventListener('click', async () => {
      ...
      setBusy(true);
      try {
        const result = await send(`/api/imports/${plan.token}/commit`, { updateExisting: update.checked });
        message(feedback, `Import complete. Added ${result.added}, updated ${result.updated}, skipped ${result.skipped}. Total students: ${result.total}.`);
        ...
        if (rejectedRows) {
          rejectedRows.querySelector('summary').textContent = `${result.skipped} rows were not imported — correct these in Excel`;
        ...
        // Any failed commit requires a fresh preview before another attempt.
        plan = null;
        preview.innerHTML = '';
  ...
  update.addEventListener('change', async () => {
    ...
    try {
      ...
      showPlan();
    } catch (error) { plan = null; preview.innerHTML = ''; message(feedback, error.message, true); }
    finally { setBusy(false); }
  });
...
export function mountDatabase(main) {
  ...
  async function load() {
    ...
    } catch (error) { if (current !== generation || !main.contains(results)) return; results.textContent = ''; message(feedback, error.message, true); }
  }
  search.addEventListener('submit', event => { event.preventDefault(); query = Object.fromEntries(new FormData(search)); page = 1; message(feedback, ''); load(); });
  search.addEventListener('reset', () => { query = { registration: '', name: '' }; page = 1; message(feedback, ''); load(); });
  async function openRecord(id) {
    ...
    try {
      ...
      dialog.showModal();
    } catch (error) { if (current === opening && main.contains(dialog)) message(feedback, error.message, true); }
  }
  ...
  recordForm.addEventListener('submit', async event => {
    ...
    try {
      ...
      load();
    } catch (error) { message(recordForm.querySelector('.feedback'), error.message, true); }
  ...
  recordForm.querySelector('#confirm-delete').addEventListener('click', async event => {
    if (recordBusy) return;
    setRecordBusy(true);
    try {
